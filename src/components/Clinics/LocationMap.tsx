...
export default function LocationMap({ locations }: LocationMapProps) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full">
      ...
      <div
        className="bg-blue-100 rounded-3xl overflow-hidden relative group min-h-[300px]"
        role="region"
      ...
      >
        ...
        {/* Mock Markers */}
        <div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow-2xl animate-bounce"
          aria-hidden="true"
        >
        ...
        <div className="absolute bottom-4 left-4 right-4 bg-white/95 backdrop-blur-sm p-4 rounded-2xl shadow-xl border border-white/40 transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all">
          ...
          <div className="text-sm font-bold text-gray-800">{locations[0].address}</div>
        </div>
      </div>
        ...
        {locations.map((loc) => (
          ...
          >
            ...
            <div className="relative">
              <h4 className="font-black text-blue-900 text-lg mb-4">{loc.name}</h4>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-600 font-bold">{loc.phone}</span>
                </div>

                <div className="flex items-center gap-3">
                  <Mail className="w-4 h-4 text-blue-500 shrink-0" />
                  <span className="text-sm text-gray-600 font-medium">{loc.email}</span>
                </div>
              </div>
