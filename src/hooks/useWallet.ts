...
export function useWallet() {
  ...
  const sendPayment = useCallback(
    async (
      ...
      /** Idempotency key generated by the TransactionSigning component / hook.
       *  Forwarded to walletService so it can attach it as an HTTP header when
       *  broadcasting to the backend, giving end-to-end duplicate protection. */
      ...
      setError(null);
      try {
        const result = await walletService.sendPayment(selectedWallet, pin, tx, idempotencyKey);
