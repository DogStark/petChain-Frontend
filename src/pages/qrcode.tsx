...
function QRCard({
  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow border p-5 flex flex-col gap-4 ${!qr.isActive ? 'opacity-60' : ''}`}
    >
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="text-xs text-gray-400 font-mono truncate max-w-[180px]">{qr.qrCodeId}</p>
          <p className="text-xs text-gray-500 mt-1">
            Scans: <span className="font-bold text-gray-800">{qr.scanCount}</span>
          </p>
        </div>
        <span
          className={`text-xs font-bold px-2 py-1 rounded-full ${qr.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}
        >
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button
          onClick={() => onSelect(qr)}
          className="flex items-center gap-1 text-xs px-3 py-2 rounded-full bg-gray-900 text-white hover:bg-gray-700 transition-all"
        >
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold flex items-center gap-2">
            <BarChart2 size={20} /> QR Analytics
          </h2>
              ...
              <div>
                ...
                <div className="space-y-1">
                  {analytics.recentScans.slice(0, 5).map((s) => (
                    <div
                      key={s.id}
                      className="text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2 flex justify-between"
                    >
...
export default function QRCodePage() {
  ...
  return (
    <ProtectedRoute>
      ...
      {selectedQR && <AnalyticsPanel qrCodeId={selectedQR} onClose={() => setSelectedQR(null)} />}
    </ProtectedRoute>
  );
}
