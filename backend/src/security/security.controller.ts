...
export class SecurityController {
  ...
  async acknowledgeAlert(
    ...
    const success =
      await this.securityMonitoringService.acknowledgeAlert(alertId);
    return { success };
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'IP address removed from blacklist successfully',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Response rules retrieved successfully',
  })
  ...
  async testThreatDetection(@Body() testRequest: any) {
    const analysis =
      await this.threatDetectionService.analyzeRequest(testRequest);
    return {
      analysis,
