...
import { MigrationService, MigrationStatus, MigrationResult, RollbackResult } from './migration.service';
...
export class MigrationController {
  ...
  @ApiQuery({ name: 'transaction', required: false, type: Boolean, description: 'Run migrations in a transaction' })
  async runMigrations(@Query('transaction') transaction?: string): Promise<MigrationResult[]> {
    const useTransaction = transaction !== 'false';
    this.logger.log(`Starting migration execution (transaction: ${useTransaction})`);
    
    try {
      const results = await this.migrationService.runMigrations({ transaction: useTransaction });
      
      const successCount = results.filter(r => r.success).length;
      const failureCount = results.filter(r => !r.success).length;
      
      this.logger.log(`Migration execution completed: ${successCount} success, ${failureCount} failed`);
  ...
  async rollbackLast(): Promise<RollbackResult> {
    ...
    try {
      ...
      if (result.success) {
        this.logger.log(`Rollback completed: ${result.rolledBack.length} migrations rolled back`);
      } else {
  ...
  @ApiResponse({ status: 200, description: 'Rollback to version completed successfully' })
  @ApiResponse({ status: 400, description: 'Invalid target version' })
  @ApiResponse({ status: 500, description: 'Rollback failed' })
  @ApiQuery({ name: 'version', required: true, type: String, description: 'Target migration version' })
  async rollbackToVersion(@Query('version') version: string): Promise<RollbackResult> {
    ...
    try {
      ...
      if (result.success) {
        this.logger.log(`Rollback to version ${version} completed: ${result.rolledBack.length} migrations rolled back`);
      } else {
        this.logger.error(`Rollback to version ${version} failed`, result.error);
      }
  ...
  @ApiResponse({ status: 201, description: 'Migration file generated successfully' })
  @ApiResponse({ status: 400, description: 'Invalid migration name' })
  async generateMigration(@Body('name') name: string): Promise<{ filePath: string }> {
    if (!name || typeof name !== 'string' || name.trim().length === 0) {
      throw new Error('Migration name is required and must be a non-empty string');
    }

    this.logger.log(`Generating new migration: ${name}`);
    
    try {
      const filePath = await this.migrationService.generateMigration(name.trim());
  ...
  async validateMigrations(): Promise<{ valid: boolean; errors: string[] }> {
    ...
    try {
      ...
      if (result.valid) {
        ...
        this.logger.warn(`Migration validation failed with ${result.errors.length} errors`);
