...
export const emailConfig = registerAs('email', () => ({
  ...
  queue: {
    ...
    intervalSeconds: parseInt(
      process.env.EMAIL_QUEUE_INTERVAL_SECONDS ?? '30',
      10,
    ),
    /** Base delay in seconds before first retry */
    retryBaseDelaySeconds: parseInt(
      process.env.EMAIL_RETRY_BASE_DELAY_SECONDS ?? '60',
      10,
    ),
    /** Multiplier applied to delay on each subsequent retry (exponential backoff) */
    retryBackoffMultiplier: parseInt(
      process.env.EMAIL_RETRY_BACKOFF_MULTIPLIER ?? '3',
      10,
    ),
