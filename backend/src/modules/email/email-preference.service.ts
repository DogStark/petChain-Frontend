...
export class EmailPreferenceService {
    ...
    if (record.userId) {
      ...
      if (record.emailType) {
        ...
        if (prefKey) {
          ...
          this.logger.log(
            `User ${record.userId} unsubscribed from ${record.emailType}`,
          );
          return {
            message: `You have been unsubscribed from ${record.emailType} emails.`,
          };
        }
      }
    }
  ...
  private emailTypeToPrefKey(
    type: EmailType,
  ): keyof UpdatePreferencesDto | null {
