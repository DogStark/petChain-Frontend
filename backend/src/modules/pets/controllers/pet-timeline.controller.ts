...
export class PetTimelineController {
  ...
  async shareTimeline(
    ...
    const share = await this.timelineService.createTimelineShare(
      petId,
      user.id,
      shareDto,
    );
   ...
   */
  private generateTimelinePdf(
    timeline: PetTimelineResponseDto,
  ): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc.moveDown(0.5);
      doc.fontSize(14).text(`${timeline.petName}`, { align: 'center' });
      doc.moveDown();
      ...
      if (timeline.filters.startDate || timeline.filters.endDate) {
        const dateRange = [
          timeline.filters.startDate
            ? `From: ${timeline.filters.startDate}`
            : '',
      ...
      for (const event of timeline.events) {
        ...
        doc
          .fontSize(10)
          .fillColor('#666666')
          .text(dateStr, 50, doc.y, { continued: true, width: 60 });

        // Event type badge
        doc
          .fillColor(color)
          .text(` [${event.type.toUpperCase()}] `, { continued: true });
        ...
        if (event.description) {
          doc
            .fontSize(9)
            .fillColor('#666666')
            .text(`    ${event.description}`, { indent: 70 });
        }
