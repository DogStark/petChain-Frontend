...
describe('MedicalRecordsController', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: MedicalRecordsExportService, useValue: mockExportService },
        {
          ...
          useValue: { getUserRoles: jest.fn(), getUserPermissions: jest.fn() },
        },
        {
          provide: PermissionsService,
          useValue: { checkPermissionAccess: jest.fn().mockReturnValue(true) },
        },
      ],
    }).compile();
  ...
  describe('create', () => {
    it('should create a medical record', async () => {
      ...
      expect(await controller.create(dto as any, 'user-1')).toEqual(result);
    ...
    it('should handle file uploads on create', async () => {
      ...
      mockService.saveAttachment.mockResolvedValue(
        'uploads/medical-records/uuid-xray.jpg',
      );
      mockService.create.mockResolvedValue({
        id: 'rec-1',
        ...dto,
        attachments: ['uploads/medical-records/uuid-xray.jpg'],
      });

      const result = await controller.create(dto as any, 'user-1', files);
  ...
  describe('remove', () => {
    it('should remove a record', async () => {
      ...
      expect(mockService.remove).toHaveBeenCalledWith('rec-1', 'user-1');
    });
  });
  ...
  describe('getTemplates', () => {
    it('should return templates for a pet type', async () => {
      ...
      expect(mockService.getTemplatesByPetType).toHaveBeenCalledWith(
        PetSpecies.DOG,
      );
    });
  });
