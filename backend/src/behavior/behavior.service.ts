...
export class BehaviorService {
  ...
  async create(
    petId: string,
    createBehaviorLogDto: CreateBehaviorLogDto,
  ): Promise<BehaviorLog> {
  ...
  async update(
    id: string,
    updateDto: Partial<CreateBehaviorLogDto>,
  ): Promise<BehaviorLog> {
  ...
  async getTrends(petId: string) {
    ...
    const mostCommonTrigger =
      triggers.length > 0 ? this.findMostCommon(triggers) : null;
  ...
  private detectConcerningPatterns(logs: BehaviorLog[]) {
    ...
    const recentHighSeverity = logs.filter(
      (l) =>
        (l.severity === BehaviorSeverity.HIGH ||
          l.severity === BehaviorSeverity.CRITICAL) &&
        new Date().getTime() - new Date(l.date).getTime() <
          7 * 24 * 60 * 60 * 1000,
    );
    ...
    if (aggressionLogs.length >= 3) {
      alerts.push({
        ...
        severity: 'HIGH',
        message:
          'Multiple aggression incidents recorded. Consult a professional.',
